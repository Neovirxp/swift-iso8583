...
class ISOHelper {
    ...
    class func binaryToHexAsString(binaryString: String?) -> String? {
        ...
        for i in (0 ..< binaryString!.count) where i % 4 == 0 {
//        for var i = 0; i < binaryString!.count; i += 4 {
    ...
    class func fillStringWithZeroes(string: String?, fieldLength: String?) -> String? {
        ...
        let regEx = try? NSRegularExpression(pattern: regExPattern, options: [])
        let regExMatches = regEx?.numberOfMatches(in: string!, options: [], range: NSMakeRange(0, string!.count))
