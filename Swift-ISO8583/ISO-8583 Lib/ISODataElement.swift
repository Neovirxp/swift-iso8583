        ...
        if dataElementsScheme.object(forKey: name) != nil {
            ...
            if dataType == "an" || dataType == "ans" && (length as NSString).range(of: ".").location == NSNotFound {
                ...
                if (length as NSString).range(of: ".").location != NSNotFound {
                    ...
                    if (length.count == 2) {
                        let index = length.index(length.startIndex, offsetBy: 1)
                        maxLength = Int(String(length[index...]))!
                        numberOfLengthDigits = 1
                    } else if (length.count == 4) {
                        let index = length.index(length.startIndex, offsetBy: 2)
                        maxLength = Int(String(length[index...]))!
                        numberOfLengthDigits = 2
                    } else if (length.count == 6) {
                        let index = length.index(length.startIndex, offsetBy: 3)
                        maxLength = Int(String(length[index...]))!
...
//            var fromIndex = advance(value!.startIndex, 1);
            var fromIndex = value!.index(value!.startIndex, offsetBy: 1)
...
//                cleanValue = value![fromIndex...]
    ...
    private func isValidDataType(dataType: String) -> Bool {
        ...
        return (validDataTypes?.index(of: dataType))! > -1
    }
    ...
    private func isValueCompliantWithDataType(value: String, dataType: String) -> Bool {
        if dataType == "a" {
            ...
            let regExMatches = regEx!.numberOfMatches(in: value, options: [], range: NSMakeRange(0, value.count))
