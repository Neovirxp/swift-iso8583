...
class ISOBitmap {
    ...
    init?(binaryString: String?) {
        ...
        let regExMatches = regEx?.numberOfMatches(in: binaryString!, options: [], range: NSMakeRange(0, binaryString!.count))
...
//        let firstCharacterIndex = binaryString?.index(binaryString!.startIndex, offsetBy: 1)
//        let firstCharacterIndex = advance(binaryString!.startIndex, 1)
        hasSecondaryBitmap = binaryString!.hasPrefix("1")
    ...
    init?(hexString: String?) {
        ...
        let regEx = try? NSRegularExpression(pattern: regExPattern, options: [])
        let regExMatches = regEx?.numberOfMatches(in: hexString!, options: [], range: NSMakeRange(0, hexString!.count))
...
//        let firstCharacterIndex = advance(hexString!.startIndex, 1)
        hasSecondaryBitmap = hexString!.hasPrefix("8") || hexString!.hasPrefix("9") || hexString!.hasPrefix("A") || hexString!.hasPrefix("B") || hexString!.hasPrefix("C") || hexString!.hasPrefix("D") || hexString!.hasPrefix("E") || hexString!.hasPrefix("F")
    ...
    init?(givenDataElements: [String]?, customConfigFileName: String?) {
        ...
        for dataElement in givenDataElements! {
            ...
            if hasSecondaryBitmap {
                ...
                let index = bitmapTemplateAsString!.index(bitmapTemplateAsString!.startIndex, offsetBy: 64)
                rawValue = String(bitmapTemplateAsString![...index])
        ...
        for i in 0...binaryBitmap!.count {
            ...
            if bit == "1" {
                ...
                if customConfigFileName != nil {
                    ...
                    let sortDescriptor = NSSortDescriptor(key: String(), ascending: true, comparator: {(object1: AnyObject!, object2: AnyObject!) -> ComparisonResult in
                        let string1 = object1 as! String
                        let string2 = object2 as! String
                        return string1.compare(string2, options: NSString.CompareOptions.numeric)
                        } as! Comparator)
